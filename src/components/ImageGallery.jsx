      ...
      <section className="flex gap-2">
        {images.map((image, id) => (
          <img
            src={image}
            key={id}
            className="w-60 cursor-pointer hover:scale-125 transition-all rounded-lg"
            onClick={() => setVal(id)}
          />
        ))}
      </section>
      <section className="flex gap-6 item-center justify-center mt-10">
        {val !== -1 && <img src={images[val]} className="rounded-md" />}
      </section>
    </>
  );
}
